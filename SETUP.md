...
To publish a build, push a version tag such as `v1.0.3`; the workflow builds **Minimal Browser** and creates the matching GitHub Release. The workflow needs the repository's default `GITHUB_TOKEN` permission `contents: write`, which is declared in the workflow.
