...
3. The generated APK has application ID `com.minimal.browser`, version `1.2.6`, ARM64 native code, and no `application-debuggable` marker.
...
The APK is an `assembleRelease` non-debuggable process, but is signed with Android’s standard debug certificate on the disposable GitHub runner because this repository has no persistent private release keystore. The generated certificate is not a durable update identity, so Android may require uninstalling an older differently signed Minimal Browser build before installation; export any data first. It targets Android 8.0+ (`minSdk 26`) and intentionally carries Mozilla GeckoView's ARM64 native engine. A stable private signing key stored as GitHub secrets is required for seamless updates across releases. A successful workflow proves build, packaging, signing, and release plumbing; it does **not** prove live-device behavior. Validate the full three-dot drawer and a private tab; blank-tab search; Web navigation/scrolling; a Google/Arena login or consent handoff, including any prompt and foreground tab; tabs; a real confirmed download including opening it from the Downloads list; the Status bar setting; the precise 5-second page-only gesture; one-Back and page-double-tap restore; printing; and heat/jank behavior on an Android 14+ ARM64 device after installation. Also test a real USB **and** Bluetooth keyboard/mouse: no software IME after clicking a page text field or the native Home/omnibox field, normal physical typing into the focused page field, both Enter variants submitting Home/omnibox once, shortcuts, mouse wheel/click/context-click, and Back/Forward side buttons.
