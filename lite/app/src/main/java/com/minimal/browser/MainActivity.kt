...
import com.minimal.browser.lite.BuildConfig
import com.minimal.browser.lite.R
...
class MainActivity : AppCompatActivity(), TabManager.Host,
    ...
    private fun wireUp() {
        ...
        rail.web.onTap = { leaveFullScreenIfNeeded(); show(Screen.WEB) }
    ...
    private fun handleBack() {
        ...
        if (videoFullScreen) {
            TabManager.exitVideoFullScreen()
            return
        }
        if (fullScreen) {
    ...
    private fun newTab() {
        ...
        val tab = TabManager.newTab(target) ?: return
        TabManager.switchTo(tab)
        show(Screen.HOME)
    }

    private fun newPrivateTab() {
        val tab = TabManager.newTab(null, private = true) ?: return
        TabManager.switchTo(tab)
    ...
    override fun onEnterVideoFullScreen(fullScreen: Boolean) {
        ...
        if (fullScreen) {
            (webScreen.webView.webChromeClient as? TabManager.VideoHost)?.currentCustomView()?.let { custom ->
                (custom.parent as? android.view.ViewGroup)?.removeView(custom)
                viewport.addView(custom, FrameLayout.LayoutParams(
                    FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.MATCH_PARENT
                ))
            }
            ...
            for (i in viewport.childCount - 1 downTo 0) {
                val child = viewport.getChildAt(i)
                if (child !== homeScreen && child !== webScreen && child !== tabsScreen &&
                    child !== settingsScreen && child !== listScreen
                ) {
                    viewport.removeView(child)
                }
            }
            applyFullScreen()
        }
    ...
    override fun openFileChooser(
        ...
        val intent = Intent(Intent.ACTION_GET_CONTENT).apply {
            ...
            type = accept?.takeIf { it.isNotBlank() } ?: "*/*"
        }
        try {
    ...
    override fun onPrint() {
        val view = TabManager.active?.webView
        if (view == null) {
            toast.say("Nothing to print yet")
            return
        }
        val jobName = TabManager.active?.displayTitle?.replace(Regex("[^A-Za-z0-9 ._-]"), "")?.trim()
            ?.take(48)?.ifEmpty { "page" } ?: "page"
        val pm = getSystemService(Context.PRINT_SERVICE) as? android.print.PrintManager
        if (pm == null) {
            toast.say("Printing is unavailable")
            return
        }
        pm.print(jobName, view.createPrintDocumentAdapter(jobName), null)
        toast.say("Opening the print dialog…")
    }
    ...
    override fun onFindInPage() {
        ...
        AlertDialog.Builder(this, R.style.Theme_Minimal_Dialog)
            ...
            .setNeutralButton("Previous") { _, _ ->
                ...
                TabManager.findNext(forward = false)
            }
        ...
        AlertDialog.Builder(this, R.style.Theme_Minimal_Dialog)
            ...
            .setPositiveButton("Clear") { _, _ ->
                Thread {
                    ...
                    if (checked[2]) {
                        runOnUiThread { TabManager.active?.webView?.clearCache(true) }
    ...
    override fun onResume() {
        super.onResume()
        TabManager.attach(webScreen.container)
        applySystemUi()
        homeScreen.refresh()
    }
    ...
    override fun onConfigurationChanged(newConfig: android.content.res.Configuration) {
        ...
        // Landscape is locked, so this is a no-op — kept for completeness.
    }
}
