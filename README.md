...
- One deferred, high-performance `SurfaceView`-backed visible `GeckoView`; Home, Tabs, Settings, and hidden screens never own a compositor surface.
...
3. validates the generated process is non-debuggable, ARM64-only native code, `lib/arm64-v8a/libxul.so`, and install-time native-library extraction in the built APK;
...
4. hold Web for exactly five seconds, then verify one Back and page double-tap restore;
5. scrolling, navigation, tab switching, and device heat/stutter behavior on ordinary and tracker-heavy pages;
6. download and print flows.
