...
object TabManager {
    ...
    fun attach(v: GeckoView) {
        ...
        if (tab != null && v.session !== tab.session) {
    ...
    private fun newSession(private: Boolean): GeckoSession {
        val settings = GeckoSessionSettings().apply {
            setUseTrackingProtection(Prefs.shieldsOn)
            setSuspendMediaWhenInactive(true)
            setAllowJavascript(Prefs.javaScriptEnabled)
            setUserAgentMode(
                if (Prefs.desktopUserAgent) GeckoSessionSettings.USER_AGENT_MODE_DESKTOP
                else GeckoSessionSettings.USER_AGENT_MODE_MOBILE
            )
            setViewportMode(GeckoSessionSettings.VIEWPORT_MODE_MOBILE)
            if (private) {
                // A separate session-context id == a private window: cookies,
                // storage and cache are isolated and never touch disk history.
                try {
                    val m = GeckoSessionSettings::class.java
                        .getMethod("setContextId", String::class.java)
                    m.invoke(this, "private-" + UUID.randomUUID())
                } catch (e: Throwable) {
                    Log.w(TAG, "private context unavailable: ${e.message}")
                }
            }
        }
